...
//! ([`crate::jvm::member_set::member_set_query`]) is likewise memoized per interned
...
/// as a [`ClassKey`](crate::jvm::member::ClassKey).
...
pub(crate) fn enclosing_class_query(
    ...
    fn walk(
        ...
        map: &mut FxHashMap<ItemId, crate::jvm::member::ClassKey>,
    ) {
        ...
        if let Some(enclosing) = current {
            map.insert(
                id,
                crate::jvm::member::ClassKey::of(tree, file_id, enclosing),
            );
        }
