...
mod tests {
    ...
    /// A jar holding the given `(entry name, bytes)` pairs.
    ...
    #[test]
    fn class_bytes_are_read_back_out_of_a_jar() {
        ...
        let bytes = read_class_bytes(&jar, LibraryKind::Jar, "com/example/Foo.class").unwrap();
        assert_eq!(bytes, b"\xca\xfe\xba\xbe");

        let err = read_class_bytes(&jar, LibraryKind::Jar, "com/example/Bar.class").unwrap_err();
        assert!(
            err.to_string().contains("com/example/Bar.class"),
            "the error names the entry that is missing: {err}"
        );
    }

    #[test]
    fn cfr_argv_carries_the_classpath_in_one_argument() {
        let externals = [PathBuf::from("/lib/a.jar"), PathBuf::from("/lib/b.jar")];
        let args = argv(Cfr.args(
            Path::new("/jars/cfr.jar"),
            Path::new("/tmp/Foo.class"),
            &externals,
            Path::new("/tmp/out"),
        ));
        let classpath = std::env::join_paths(&externals).unwrap();
        assert_eq!(
            args,
            [
                "-jar",
        ...
        // With nothing on the classpath the flag is omitted rather than passed
        // empty.
        let bare = argv(Cfr.args(
            Path::new("/jars/cfr.jar"),
            Path::new("/tmp/Foo.class"),
            &[],
            Path::new("/tmp/out"),
        ...
        assert!(
            !bare.iter().any(|arg| arg == "--extraclasspath"),
            "{bare:?}"
        );
    }

    #[test]
    fn vineflower_argv_passes_one_external_per_entry() {
        let args = argv(Vineflower.args(
            Path::new("/jars/vineflower.jar"),
            Path::new("/tmp/Foo.class"),
            &[PathBuf::from("/lib/a.jar"), PathBuf::from("/lib/b.jar")],
            Path::new("/tmp/out"),
        ));
        assert_eq!(
            args,
            [
                ...
                "/jars/vineflower.jar",
                "--folder",
                "--add-external=/lib/a.jar",
                "--add-external=/lib/b.jar",
                "/tmp/Foo.class",
                "/tmp/out",
            ]
        );
    ...
    #[test]
    fn the_registry_resolves_a_known_backend_only() {
        assert_eq!(backend("cfr").map(|backend| backend.id()), Some("cfr"));
        assert_eq!(
            backend("vineflower").map(|backend| backend.id()),
            Some("vineflower")
        );
        assert!(backend("jd").is_none());
        assert!(!is_backend(""));
    }

    #[test]
    fn prepare_roots_creates_one_root_per_library_and_prunes_the_rest() {
        let dir = tempfile::TempDir::new().unwrap();
        let live = LibraryId(0xabcd);
        let stale = LibraryId(0xef01);

        let view = LibraryView::Decompiled {
            backend: "cfr".to_owned(),
        };
        let stale_root = library_view::root_dir(dir.path(), &view, stale);
        fs::create_dir_all(&stale_root).unwrap();
        // Another backend's output goes away with the switch.
        let other = library_view::root_dir(
            dir.path(),
            &LibraryView::Decompiled {
                backend: "vineflower".to_owned(),
            },
            live,
        );
        fs::create_dir_all(&other).unwrap();

        let roots = prepare_roots(dir.path(), "cfr", &[live]);
        assert_eq!(
            roots.get(&live).map(|root| root.as_ref()),
            Some(Utf8Path::from_path(&library_view::root_dir(dir.path(), &view, live)).unwrap())
        );
        assert!(roots.get(&live).is_some_and(|root| {
            let path: &Utf8Path = root.as_ref();
            path.exists()
        }));

        assert!(
            !stale_root.exists(),
            "a library off the classpath is pruned"
        );
        assert!(!other.exists(), "another backend's cache is pruned");
    }
}
