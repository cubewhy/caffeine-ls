...
mod tests {
    ...
    fn diagnostic(severity: Option<DiagnosticSeverity>) -> Diagnostic {
        Diagnostic {
            ...
            severity,
            code: Some(lsp_types::Code::String("compiler.err".into())),
            message: lsp_types::Message::String("';' expected".into()),
            ..Diagnostic::default()
        }
    }

    #[test]
    fn collect_respects_severity_threshold() {
        let diagnostics = vec![
            diagnostic(Some(DiagnosticSeverity::Error)),
            diagnostic(Some(DiagnosticSeverity::Warning)),
            diagnostic(Some(DiagnosticSeverity::Hint)),
        ];

        let errors_only = collect_entries(
            "src/A.java",
            Path::new("/nonexistent"),
            &diagnostics,
            crate::flags::SeverityFilter::Error.max_rank(),
        );
        assert_eq!(errors_only.len(), 1);
        assert_eq!(errors_only[0].severity, "error");

        let warnings_and_up = collect_entries(
            "src/A.java",
            Path::new("/nonexistent"),
            &diagnostics,
            crate::flags::SeverityFilter::Warning.max_rank(),
        );
    ...
    fn positions_are_one_based() {
        // /nonexistent cannot be read, exercising the fallback path.
        let entries = collect_entries(
            "src/A.java",
            Path::new("/nonexistent"),
            &[diagnostic(Some(DiagnosticSeverity::Error))],
            crate::flags::SeverityFilter::All.max_rank(),
        );
    ...
    fn text_render_includes_summary() {
        ...
        assert!(text.contains("src/A.java:3:9: error[compiler.err]: ';' expected"));
        assert!(text.contains("1 error(s) found across 3 analyzed file(s), 1 skipped"));
    }
}
